...
navigation_array <- function(title) {
  ...
  directions <- data.frame(
    ...
    'Current Lesson'=markdownToPrint[pageNumber], 
    'Next Lesson'=if(pageNumber < length(titles)) markdownToPrint[pageNumber+1] else NA,
    stringsAsFactors=FALSE)
  
  knitr::kable(directions, col.names=paste(c("Previous","Current","Next"), "Lesson"))
}