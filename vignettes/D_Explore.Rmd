...
directions <- dfPages$markdownToPrint[c(4,6)]
# directions[1] <- paste("<-",directions[1]) 
# directions[2] <- paste(directions[2],"->")   
directions <- c(directions[1],"-----------------------",directions[2])
kable(t(directions))
```

- [Previous lesson](C_Clean.html): Clean the data.
- [Next lesson](E_Analyze.html): Analyze the data in base R.
...
##Exercise 1
Next, we're going to explore the distribution of the NM phosphorus data using base R statistical functions

1. For this first part, explore each of the functions above (`summary()`, `range()`, `IQR()`, and `quantile()`) and get to know more about the NWIS water variables we collected and merged into our 'gages_data_wq' data frame.  Specifically, you'll want to focus on the 'results_xx' variables within the 'gages_data_wq' data frame Hint: Use the $ operator to index a particular columns of this data frame using the functions above. 

OPTIONAL: Next lets continue to add to our script.  This time we will be combining some of the past lessons with this one.  Our goal is to create a summary statistics data frame of `gages_data_wq`.  What we want is a data frame that has min, max, mean, median, IQR, and quartiles for each of the four varaibles measured at 'gages' sites in NM that we pulled from NWIS.  This may seem a bit daunting, but if we break it apart it gets a bit easier. This is a base R solution. 

2. Create a separate vector for each of the water quality variables.  The vector should be of length 7. You can choose the name of the vector; choose something descriptive that would be obvious to your neighbor if you shared the script or data frame. 

3. Make sure your column names AND row names make sense.
...
3. Lastly, add a boxplot of discharge that shows a boxplot for [NSIP Sentinel](http://water.usgs.gov/nsip/federalneeds.html) ('NSIP_SENTINEL') sites versus non-Sentinel sites. Feel free to try and log transform the discharge data (not required).
