...
3. Add commands to create two data frames: One named `gages_sites` that contains all the data in the [conterm_basinid.csv](https://usgs-r.github.io/introR/rmd_posts/all_gages_ii_data/conterm_basinid.csv) file, and another named `gages_cover` that contains all the data in the [conterm_lc06_basin.csv](https://usgs-r.github.io/introR/rmd_posts/all_gages_ii_data/conterm_lc06_basin.csv) file.
4. Create a third data.frame from [this Excel file](http://water.usgs.gov/nsip/nsipmaps/nsip_2015.xlsx), which lists the [NSIP Sentinel Sites](http://water.usgs.gov/nsip/federalneeds.html). Download the file, export the sheet to .csv, and read it into a data.frame named gages_sentinels. Add `stringsAsFactors=FALSE` as the second argument to your call to `read.csv` - this will prevent character fields from being converted to factors willy-nilly.
