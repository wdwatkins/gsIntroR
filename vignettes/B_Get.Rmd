...
```{r operators_consoloe, eval=FALSE, echo=TRUE}
...
##Exercise 2
For this exercise we are going to grab some more data, look at that data, and be able to describe some basic information about that dataset. The data we will use will come from NWIS via dataRetrieval. 

1. Create a new script in RStudio.  Name it "usgs_analysis.R"
2. As you write the script comment as you go. Also, be sure to use the help files for the dataRetrieval package to help you identify arguments and functions that you need to use. 
3. Using a site number of your choice (or 01010000 if you can't think of one), create two data frames: one with phosphorus data (P Code = 00665), and one with discharge (P Code = 00060).  
4. Add commands to your script that will provides details on the structure (hint: `str`) of each newly created data frame. Are the data frames the same size?
6. Run the script and make sure it doesn't throw any errors and you do in fact get the two data frames.
7. If you still have some time, explore the data frames using some of the commands we covered. Or, explore more dataRetrieval functions and arguments.
