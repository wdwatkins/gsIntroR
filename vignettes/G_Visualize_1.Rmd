---
title: "G. Visualize - Advanced Plotting Part I"
...
vignette: >
  ...
  %\VignetteIndexEntry{G. Visualize}
...
###Plot Styles
So far we have learned to plot data using scatter plots, boxplots, and histograms for exploratory purposes. But what if you wanted to present this data to others? Usually, that would require some work on its style, and base R plotting has a many features that can make this process easy. 
...
Similarly, you can change linetypes (`lty`), linewidths (`lwd`), point size (`cex`). These all refer to graphical parameters and there are many more. Use `?par` to read about the others.  
...
1. Copy and paste the following code to have a data frame of USGS phosphorus data. 
...
It is very simple to save your plots as images. In RStudio's "Plots" window, you can select the "Export" drop down and say "Save as Image" or "Save as PDF". Alternatively, you can use the following functions: `png()`, `jpeg()`, `pdf()`, `svg()`, and the list goes on. To use, just specify what the file name or file path (there are other arguments if you need something more specific, look at `?png`). 

In order to view the image you just saved, you will have to make sure that this image isn't open as a current device. To do so, use `dev.off()`. The default is to turn off the most current device, but you can specify which device by using the argument `which = ` inside the function. 
